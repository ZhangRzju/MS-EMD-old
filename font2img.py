...
def img_resize(img, size=80):
    ...
    if h > w:       # 横向填充
        k = h - w
        pad_img = np.zeros((h, k//2))
        pad_img[:, :] = 255
        new_img = np.concatenate([pad_img, img], axis=1)
        pad_img = np.zeros((h, k - k//2))
        pad_img[:, :] = 255
        new_img = np.concatenate([new_img, pad_img], axis=1)
    else:           # 纵向填充
        k = w - h
        pad_img = np.zeros((k//2, w))
        pad_img[:, :] = 255
        new_img = np.concatenate([pad_img, img], axis=0)
        pad_img = np.zeros((k - k//2, w))
        pad_img[:, :] = 255
        new_img = np.concatenate([new_img, pad_img], axis=0)
...
def font2img(img_path, font_path, resol):
    ...
    for i in tqdm(range(len(font_list))):
        ...
        for j in range(len(char_list)):
            ...
            cv2.imwrite(path + '/' + '%04d' % j + '.png', img_resize(img, resol))
